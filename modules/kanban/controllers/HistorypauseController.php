<?php
/**
 * HistorypauseController
 * @var $this HistorypauseController
 * @var $model KanbanTaskHistoryPause * @var $form CActiveForm
...
class HistorypauseController extends Controller
{
	...
	*/
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new CActiveDataProvider('KanbanTaskHistoryPause', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
	...
	{
		$model=new KanbanTaskHistoryPause;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['KanbanTaskHistoryPause'])) {
			$model->attributes=$_POST['KanbanTaskHistoryPause'];
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-kanban-task-history-pause',
							'msg' => '<div class="errorSummary success"><strong>KanbanTaskHistoryPause success created.</strong></div>',
						));
					} else {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-kanban-task-history-pause',
							'msg' => '<div class="errorSummary success"><strong>KanbanTaskHistoryPause success updated.</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				KanbanTaskHistoryPause::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				KanbanTaskHistoryPause::model()->deleteAll($criteria);
			}
		}
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-kanban-task-history-pause',
						'msg' => '<div class="errorSummary success"><strong>KanbanTaskHistoryPause success deleted.</strong></div>',
					));
				}
			}
			...
			if(isset($id)) {
				...
				if($model->update()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-kanban-task-history-pause',
						'msg' => '<div class="errorSummary success"><strong>KanbanTaskHistoryPause success published.</strong></div>',
					));
				}
			}
	...
	{
		$model = KanbanTaskHistoryPause::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='kanban-task-history-pause-form') {
